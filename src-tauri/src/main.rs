...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        ...
        copyright: Some("\u{00a9} 2025 GlobalPulse Team".into()),
        website: Some("https://46.62.167.252".into()),
        website_label: Some("46.62.167.252".into()),
        ..Default::default()
    };
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/ntamero/globalpulse");
        }
        MENU_HELP_DEVTOOLS_ID => {
