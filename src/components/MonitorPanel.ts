...
export class MonitorPanel extends Panel {
  ...
  public renderResults(news: NewsItem[]): void {
    ...
    // Track which monitors matched each news item (by link)
    const matchMap = new Map<string, { item: NewsItem; colors: string[] }>();

    news.forEach((item) => {
      this.monitors.forEach((monitor) => {
        // Search title and any available text content
        const searchText = `${item.title} ${item.source}`.toLowerCase();
        const matched = monitor.keywords.some((kw) => {
          if (kw.length <= 2) {
            // Short keywords: exact word boundary match to avoid false positives
            const escaped = kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const regex = new RegExp(`\\b${escaped}\\b`, 'i');
            return regex.test(searchText);
          }
          // Longer keywords: simple includes is sufficient and more flexible
          return searchText.includes(kw);
        });
        if (matched) {
          const existing = matchMap.get(item.link);
          if (existing) {
            if (!existing.colors.includes(monitor.color)) {
              existing.colors.push(monitor.color);
            }
          } else {
            matchMap.set(item.link, { item: { ...item, monitorColor: monitor.color }, colors: [monitor.color] });
          }
        }
      });
    });

    const unique = Array.from(matchMap.values());
    ...
    results.innerHTML = `
      ...
      ${unique
        ...
        .map(
          ({ item, colors }) => {
            // Show gradient border if multiple monitors matched
            const borderStyle = colors.length > 1
              ? `border-image: linear-gradient(to bottom, ${colors.join(', ')}) 1; border-left-width: 3px; border-left-style: solid;`
              : `border-left: 2px solid ${escapeHtml(colors[0] || '')};`;
            return `
        <div class="item" style="${borderStyle} padding-left: 8px; margin-left: -8px;">
